...
Довольно распространенный обходной путь для сайтов, избегающих необходимости внедрения системы JWT с отслеживанием состояния для выхода из системы пользователей, - это быстрое автоматическое истечение срока действия токенов.

Например, вы можете поместить время создания JWT в его собственную полезную нагрузку (payload) и / или запланированное время истечения. Затем, при проверке входящих JWT, просто проверьте, является ли время истечения ранее или равно текущему времени - если так, отклоните JWT.

Проблема с этим заключается в том, что это все еще некое временное решение, которое может оказаться _более сложным_, чем просто использование сессий. Чтобы пользователи не выходили из системы во время активного использования сайта (это раздражает!), Вам необходимо использовать автообновление JWT, возможно, с помощью службы, подобной токенам обновления Auth0 [Auth0's Refresh Tokens](https://auth0.com/docs/tokens/refresh-token/current). Это по-прежнему не допускает ручного выхода из системы, если только вы не вводите отзыв в качестве части шаблона обновления (refresh pattern).

## Предотвращение state: ленивое анулирование (Lazy invalidation).

Представьте, что вам нужно существенно обновить систему управления пользователями и заставить всех переключаться. Обходной путь, который, к сожалению, предлагается многими разработчиками в Интернете, но далеко не идеален, состоит в том, чтобы ... изменить свой секретный ключ. Если вы сделаете это, вы _немедленно_ лишите законной силы **все JWT** в обращении и заставите всех войти снова, так как часть подписи JWT каждого не будет больше соответствовать.

## Обратите внимание на сложность, уязвимости и многое другое:
